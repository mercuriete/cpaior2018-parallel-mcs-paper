...
With one exception, each plot gives the same conclusion: if we are working with a timeout of at
...
consider instances where the sequential algorithm succeeded, and so these plots therefore underestimate
...
\begin{figure}[p]
    ...
    also with 32 threads. ?? 8 threaded results still running}\label{figure:cliquescale}
\end{figure}
