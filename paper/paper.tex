...
Rather than storing a domain for each vertex in V(G) -- which results in redundant storage in cases
where domains are identical -- equivalence classes of vertices in V(G) are stored, and a set of
candidate vertices in H is stored for each equivalence class. This enables fast propagation of the
constraints and smaller memory requirements. In addition, the data structure enables good branching
heuristics to be calculated cheaply.
