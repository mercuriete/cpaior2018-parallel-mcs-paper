...
namespace
{
    ...
    struct SIP
    {
            ...
            all_wildcards(domain_size)
        {
            ...
            while (true) {
                if (params.nds) {
                    ...
                    cilk_for (unsigned t = 0 ; t < target.size() ; ++t) {
                        unsigned cn = 0;
                        for (auto & c : adjacency_constraints) {
                            for (unsigned q = 0 ; q < target.size() ; ++q)
                                if (c.second[t][q])
        ...
        auto solve(
                ...
                unsigned depth,
                reducer<op_add<unsigned long long> > & my_nodes) -> void
        {
            ...
            *my_nodes += 1;
            ...
            for (auto & branch_value : branch_values) {
                ...
                ++i;
                funcs.emplace_back([&domains, branch_domain, branch_value, assignments, depth, i, &my_nodes, this] () mutable {
                    ...
                    if (unit_propagate(new_domains, assignments)) {
                        solve(new_domains, assignments, depth + 1, my_nodes);
                    }
                });
            }
        ...
        auto run()
        {
            ...
            if (unit_propagate(initial_domains, assignments)) {
                ...
                reducer<op_add<unsigned long long> > nodes_reducer;
                solve(initial_domains, assignments, 0, nodes_reducer);
                result.nodes += nodes_reducer.get_value();
            }
...
auto cilk_subgraph_isomorphism(const pair<Graph, Graph> & graphs, const Params & params) -> Result
{
...
auto cilk_ix_subgraph_isomorphism(const pair<Graph, Graph> & graphs, const Params & params) -> Result
{
