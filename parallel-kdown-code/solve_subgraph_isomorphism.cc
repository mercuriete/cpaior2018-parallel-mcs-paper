...
auto main(int argc, char * argv[]) -> int
{
    auto subgraph_isomorphism_algorithms = {
        std::make_pair( std::string{ "parallel" },         parallel_subgraph_isomorphism ),
        std::make_pair( std::string{ "parallelix" },       parallel_ix_subgraph_isomorphism )
    };
    ...
    try {
        ...
        if (options_vars.count("threads"))
            params.threads = options_vars["threads"].as<int>();
        else
